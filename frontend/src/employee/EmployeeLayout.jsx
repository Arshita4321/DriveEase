...
const links = [
  { to: '/employee', label: 'Dashboard', icon: FiGrid, end: true },
  { to: '/employee/bookings', label: 'Bookings', icon: FiCalendar },
  { to: '/employee/vehicles', label: 'Vehicles', icon: FiTruck },
];
...
export default function EmployeeLayout() {
  ...
  const SidebarContent = (
    <>
      ...
      {/* Quick info card */}
      <div className="mt-8 rounded-xl bg-gradient-to-br from-primary-50 to-cyan-50 p-4 dark:from-white/5 dark:to-white/[0.02]">
        <div className="flex items-center gap-2 text-primary-700 dark:text-slate-300">
          <FiClock size={14} />
          <span className="text-xs font-semibold">Operations Hub</span>
        </div>
        <p className="mt-1.5 text-[11px] leading-relaxed text-primary-500 dark:text-slate-400">
          Manage daily check-ins, check-outs, and vehicle availability.
        </p>
      </div>

      <div className="mt-4 border-t border-primary-100 pt-4 dark:border-white/10">
    ...
    <div className="min-h-screen bg-surface-light dark:bg-surface-dark">
      <div className="mx-auto flex max-w-[1600px]">
        ...
        <div className="fixed inset-x-0 top-0 z-40 flex h-14 items-center justify-between border-b border-primary-100 bg-white/90 px-4 backdrop-blur-md dark:border-white/10 dark:bg-surface-dark/90 lg:hidden">
          <span className="font-display font-bold text-primary-950 dark:text-white">Employee panel</span>
